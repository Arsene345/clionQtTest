    ...
    ui(new Ui::Notepad)
{
    ...
    connect(ui->actionColor, &QAction::triggered, this, &Notepad::setFontColor);
...
void Notepad::selectFont()
{
    ...
    //le mergeCurrentCharFormat permet de mettre à jour le format du texte sélectionné
}
...
// On change la couleur du texte (pas gardée dans le fichier quand on enregistre)
void Notepad::setFontColor() {
    QColor colorTemp = QColorDialog::getColor(Qt::black, this, "Choisir une couleur");
    if (colorTemp.isValid()) {
        QTextCharFormat format;
        format.setForeground(colorTemp);
        ui->textEdit->mergeCurrentCharFormat(format);
    }
